...
def sub_values(in_list,ix):
    ...
    if len(rules_list[val]) == 1:  #  there's only one element
        ...
        if isinstance(rules_list[val],list):  # there are two lists
            ins0 = []
            ins0.append(rules_list[val][0])
            ins1 = []
            ins1.append(rules_list[val][1])
...
with open("Advent of Code problem 19 - Monster message data.txt") as mm_file:
    ...
    while not_all_strings(rules_list[0]):
        new_value = process_list(rules_list[0])
        if isinstance(new_value,str):
            valid_strings.append(new_value)
            break
        rules_list[0] = new_value
